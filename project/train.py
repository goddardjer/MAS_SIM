...
from model import ActorCriticLSTM, ActorCriticFeedforward
...
def train(
    ...
    # model = ActorCriticLSTM(obs_size, action_size).to(device)
    model = ActorCriticFeedforward(obs_size, action_size).to(device)
    optimizer = optim.Adam(model.parameters(), lr=lr)

    # Initialize hidden states for LSTM
    # hx, cx = model.init_hidden_states(num_envs * n_agents, device)
    ...
    for update in range(num_updates):
        for step in range(n_steps):
            ...
            # with torch.no_grad():
            #     dist, value, hx, cx = model(current_obs, hx, cx)
            #     action = dist.rsample()  # Use reparameterization trick for sampling
            ...
            # Check and adjust `done` to match `dones[step]`
            ...
            if done.shape[0] == num_envs:
                done = done.repeat_interleave(n_agents)  # Repeat each element `n_agents` times
        ...
        with torch.no_grad():
            _, next_value, _, _ = model(current_obs, hx, cx)
        ...
        for epoch in range(epochs):
            ...
            for start in range(0, b_obs.size(0), minibatch_size):
                ...
                # Initialize minibatch hidden states
                mb_size = batch_obs.size(0)
                mb_hx, mb_cx = model.init_hidden_states(mb_size, device)

                # Forward pass for action log probs and values
                dist, values_pred, _, _ = model(batch_obs, mb_hx, mb_cx)
    ...
    # Save the model
    torch.save(model.state_dict(), 'ppo_lstm_model.pth')
